...
def build_rental_response(rental_data):
...
def get_rentals(request):
    ...
    if not username:
        return JsonResponse({"message": "X-User-Name header is required"}, status=400)
    ...
    for rental in rental_items:
        result.append(build_rental_response(rental))
...
def get_rental_by_uid(request, rental_uid):
    ...
    result = build_rental_response(rental_response.json())
...
def create_rental(request):
    ...
    car_response = requests.get(f'{CARS_SERVICE_URL}/api/v1/cars/{car_uid}')
    ...
    reserve_response = requests.post(f'{CARS_SERVICE_URL}/api/v1/cars/{car_uid}/reserve')
    ...
    if payment_response.status_code != 201:
        requests.post(f'{CARS_SERVICE_URL}/api/v1/cars/{car_uid}/release')
        return JsonResponse({"message": "Cannot create payment"}, status=500)
    ...
    if rental_response.status_code != 201:
        requests.post(f'{CARS_SERVICE_URL}/api/v1/cars/{car_uid}/release')
        requests.post(f'{PAYMENT_SERVICE_URL}/api/v1/payment/{payment_data["paymentUid"]}/cancel')
...
def cancel_rental(request, rental_uid):
    ...
    requests.post(f'{CARS_SERVICE_URL}/api/v1/cars/{rental_data["carUid"]}/release')
    requests.post(f'{PAYMENT_SERVICE_URL}/api/v1/payment/{rental_data["paymentUid"]}/cancel')
